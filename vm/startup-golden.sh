...
# settings.json from the seed would shadow the defaults. Re-assert them here per real home dir
# via the baked claude-apply-settings helper (merges, so other keys survive). Runs each boot;
# a user's own /model or /config change still holds for the running session. The `command -v`
# guard skips nodes from an image baked before the helper existed (they get it on re-bake).
if command -v claude-apply-settings >/dev/null 2>&1; then
  for _home in /home/*; do
    [ -d "$_home/.local" ] || [ -d "$_home/.claude" ] || continue    # a real user home
    _u="$(stat -c '%U' "$_home")"
    install -d -o "$_u" -g "$_u" -m 0755 "$_home/.claude"
    claude-apply-settings "$_home/.claude/settings.json" || true
